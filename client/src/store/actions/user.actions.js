...
import { loadState } from "components/product/prodNfo";
...
export const removeFromCart = (position) => {
    return async (dispatch, getState) => {
        try {
          ...
          dispatch(actions.successGlobal("Item is Removed from Cart"));
        } catch (error) {
          dispatch(actions.errorGlobal(error.response.data.message));
        }
      };
...
export const removePersistFromCart = (position) => {
    return async (dispatch, getState) => {
        try {
          const cart = persistedState.cart;
//getState().persistedState.cart
          console.log("CCC",cart)
    
          cart.splice(position, 1);
          localStorage.setItem('appState', JSON.stringify(cart));
//use removeItem
          console.log("SSSSSS",cart.splice(position, 1))
